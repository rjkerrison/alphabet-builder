function cleanup(alphabet, letter, bestSeed) {
  for (var seed of alphabet.getSeeds(letter)) {
    ...
    var allLettersCoveredTwice = seed.everyLetters(
      (seedLetter) => alphabet.getSeeds(seedLetter).size > 1
    )
    ...
    if (allLettersCoveredTwice) {
      seed.everyLetters(
        (seedLetter) =>
          console.log(
            seedLetter,
            alphabet.getSeeds(seedLetter).size,
            Array.from(alphabet.getSeeds(seedLetter)).map((seed) => seed.name)
          ) || true
      )
      console.log(
        `${seed.name} is unnecessary now that ${bestSeed.name} has been added.`
      )
...
function operate(alphabet, chooseBestSeed) {
  ...
  console.log(
    usedSeeds.map((x) => {
      return {
        name: x.name,
        uniqueLetters: x.filterLetters(
          (letter) => alphabet.getSeeds(letter).size == 1
        ),
      }
    })
  )
  ...
  console.log(
    `${alphabet.filter((z) => z.found).length} letters found in ${
      usedSeeds.length
    } seeds.`
  )
}
